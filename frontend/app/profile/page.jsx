...
export default function ProfilePage() {
    ...
    // Avoid hydration mismatch: the server renders at a different instant (and
    // potentially timezone) than the client, so `new Date()` must not be read
    // directly during render. Compute it once on the client after mount.
    const [greeting, setGreeting] = useState("Morning");
    const [todayLabel, setTodayLabel] = useState("");
    useEffect(() => {
        const now = new Date();
        const hours = now.getHours();
        setGreeting(hours < 12 ? "Morning" : hours < 18 ? "Afternoon" : "Evening");
        setTodayLabel(now.toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric" }));
    }, []);
